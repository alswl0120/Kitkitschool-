...
import WoodenPuzzlesEngine from '../../game/woodenpuzzles/WoodenPuzzlesEngine'
import ProgressBar from '../../components/ProgressBar'
import BackButton from '../../components/BackButton'
import { useShellParams } from '../../hooks/useShellParams'

export default function WoodenPuzzlesPage() {
  const navigate = useNavigate()
  const { shellLevel, isFromShell, onGameComplete, shellBack } = useShellParams()
  const canvasRef = useRef<HTMLCanvasElement>(null)
  const engineRef = useRef<WoodenPuzzlesEngine | null>(null)
  ...
  useEffect(() => {
    fetch('/data/games/woodenpuzzles.json')
      .then(r => r.json())
      .then(data => {
        const levels = Array.from(new Set((data.levels as { level: number }[]).map(p => p.level)))
          .sort((a, b) => a - b)
  ...
  useEffect(() => {
    ...
    const engine = new WoodenPuzzlesEngine(canvas, level)
  ...
  const btnColors = ['#FF8A65','#FFB74D','#FFD54F','#AED581','#4DB6AC','#4FC3F7','#7986CB','#BA68C8','#F06292','#A1887F']
        ...
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', justifyContent: 'center', maxWidth: 600 }}>
          {availableLevels.map((lvl, i) => (
            <button key={lvl} onClick={() => startLevel(lvl)} style={{
              ...
              background: btnColors[i % btnColors.length],
