...
import { MissingNumberEngine } from '../../game/missingnumber/MissingNumberEngine'
...
export default function MissingNumberPage() {
  ...
  const engineRef = useRef<MissingNumberEngine | null>(null)
  ...
  useEffect(() => {
    fetch('/data/games/missingnumber.json')
      .then(r => r.json())
      .then(data => {
      ...
      .catch(() => {
        setAvailableLevels(Array.from({ length: 37 }, (_, i) => i + 1))
      })
  }, [])
  ...
  useEffect(() => {
    ...
    const engine = new MissingNumberEngine(canvas, level)
