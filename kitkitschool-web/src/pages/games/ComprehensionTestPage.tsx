...
import { ComprehensionTestEngine } from '../../game/comprehensiontest/ComprehensionTestEngine'
...
export default function ComprehensionTestPage() {
  ...
  const engineRef = useRef<ComprehensionTestEngine | null>(null)
  ...
  useEffect(() => {
    fetch('/data/games/comprehensiontest.json')
      .then(r => r.json())
      .then(data => {
  ...
  useEffect(() => {
    ...
    const engine = new ComprehensionTestEngine(canvas, level)
  ...
  const btnColors = ['#00695C','#00796B','#00897B','#009688','#26A69A','#4DB6AC','#00695C','#00796B','#00897B','#009688']
        ...
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', justifyContent: 'center', maxWidth: 600 }}>
          {availableLevels.map((lvl, i) => (
            <button key={lvl} onClick={() => startLevel(lvl)} style={{
              ...
              background: btnColors[i % btnColors.length], color: '#fff',
