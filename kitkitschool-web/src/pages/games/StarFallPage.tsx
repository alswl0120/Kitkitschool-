...
import { StarFallEngine } from '../../game/starfall/StarFallEngine'
...
export default function StarFallPage() {
  ...
  const engineRef = useRef<StarFallEngine | null>(null)
  ...
  useEffect(() => {
    fetch('/data/games/starfall.json')
      .then(r => r.json())
      .then(data => {
        // C++ JSON format: levels is an array with level_number field
        const lvls = (data.levels as { level_number: number }[])
  ...
  useEffect(() => {
    ...
    const engine = new StarFallEngine(canvas, level)
