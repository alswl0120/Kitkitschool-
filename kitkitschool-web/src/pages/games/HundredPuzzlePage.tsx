...
import HundredPuzzleEngine from '../../game/hundredpuzzle/HundredPuzzleEngine'
import ProgressBar from '../../components/ProgressBar'
import BackButton from '../../components/BackButton'
import { useShellParams } from '../../hooks/useShellParams'

export default function HundredPuzzlePage() {
  const navigate = useNavigate()
  const { shellLevel, isFromShell, onGameComplete, shellBack } = useShellParams()
  const canvasRef = useRef<HTMLCanvasElement>(null)
  const engineRef = useRef<HundredPuzzleEngine | null>(null)
  ...
  useEffect(() => {
    fetch('/data/games/hundredpuzzle.json')
      .then(r => r.json())
      .then(data => {
      ...
      .catch(() => {
        // Fallback to 1-10 if fetch fails
        setAvailableLevels(Array.from({ length: 10 }, (_, i) => i + 1))
      })
  }, [])
  ...
  useEffect(() => {
    ...
    const engine = new HundredPuzzleEngine(canvas, level)
